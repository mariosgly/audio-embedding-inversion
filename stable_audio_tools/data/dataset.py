...
def get_audio_filenames(
    ...
    # If a filelist is provided, read paths from it (tab-separated, first column is path)
    if filelist is not None:
        print(f'Reading filelist from {filelist}')
        with open(filelist, 'r') as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                filepath = line.split('\t')[0]  # first column only
                filenames.append(filepath)
                #for filelist skip this we do not need it
    ...
    # Otherwise scan directories as before
    ...
    for path in paths:
        ...
        print(f'Scanned {path}: {len(files)} files found ({len(filenames)} total so far)')
    
    print(f'Done. Found {len(filenames)} files total')
    return filenames
...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        augment_phase=True,
        crop_hop_seconds=None,
        crop_within_chunk_seconds=None,
        crop_within_chunk_hop_seconds=None,
    ):
        super().__init__()
        self.filenames = []
        ...
        self.pad_crop = PadCrop_Normalized_T(
            ...
            hop_seconds=crop_hop_seconds,
            crop_within_chunk_seconds=crop_within_chunk_seconds,
            crop_within_chunk_hop_seconds=crop_within_chunk_hop_seconds,
        )
        ...
        for config in configs:
            ...
            self.filenames.extend(
                get_audio_filenames(
                    config.path, 
                    keywords, 
                    skip_dirs=config.skip_dirs,  # <-- use per-config skip_dirs instead of hardcoded
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Check for silence
            if is_silence(audio):
                print(f"Audio {audio_filename} is silence")
                return self[random.randrange(len(self))]
            
            if is_mostly_silence(audio):
                print(f"Audio {audio_filename} is mostly silence")
                return self[random.randrange(len(self))]
            ...
            for custom_md_path in self.custom_metadata_fns.keys():
                ...
                if "__reject__" in info and info["__reject__"]:
                    print('something rejected')
                    print(f"Couldn't load file {audio_filename}")
...
def is_mostly_silence(
    audio,       # torch tensor of (multichannel) audio
    thresh=0.0001, # RMS threshold. 0.01 is approx -40dB. Adjust if needed.
    ):
    """
    Checks if entire clip is 'mostly silent' using RMS energy.
    """
    # Calculate Root Mean Square
    rms = torch.sqrt(torch.mean(audio ** 2))
    return rms.item() < thresh
...
class WebDatasetDataLoader():
    def __init__(
        ...
        **data_loader_kwargs
        ...
        self.random_crop = random_crop
        self.crop_hop_seconds = crop_hop_seconds
        self.force_channels = force_channels
        ...
        if self.pre_encoded:
            ...
            if self.sample_size is not None:
                ...
                pad_crop = PadCrop_Normalized_T(
                    ...
                    hop_seconds=self.crop_hop_seconds,
                )
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    if dataset_type == "audio_dir":
        ...
        for audio_dir_config in audio_dir_configs:
            ...
            filelist = audio_dir_config.get("filelist", None) 
            ...
            configs.append(
                LocalDatasetConfig(
                    ...
                    filelist=filelist,
                    skip_dirs=audio_dir_config.get("skip_dirs", []), 
                )
            )

        # print('now calling the SampleDataset class')
        crop_within_chunk_enabled = dataset_config.get("crop_within_chunk", False)
        ...
        train_set = SampleDataset(
            ...
            augment_phase=dataset_config.get("augment_phase", True),
            crop_hop_seconds=dataset_config.get("crop_hop_seconds", None),
            crop_within_chunk_seconds=dataset_config.get("crop_within_chunk_seconds", None) if crop_within_chunk_enabled else None,
            crop_within_chunk_hop_seconds=dataset_config.get("crop_within_chunk_hop_seconds", None) if crop_within_chunk_enabled else None,
        )
