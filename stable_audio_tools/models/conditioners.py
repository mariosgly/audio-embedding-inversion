...
class SequenceEmbeddingConditioner(Conditioner):
    """
    Takes per-sample embedding sequences of shape [T_i, D_in] (variable length),
    pads them to [B, T_max, output_dim], and returns (tokens, mask).

    Expected input from metadata: a list of torch.Tensors, each [T_i, D_in].
    """
    def __init__(
        self,
        output_dim: int,
        ...
        pos_emb_max_length: int = 0,  #(fallback if max_length=0)
        small_init_proj_out: bool = False,
        proj_out_init_std: float = 1e-3,
    ):
        super().__init__(dim=in_dim, output_dim=output_dim, project_out=project_out)
        self.in_dim = in_dim
        self.max_length = max_length
        self.l2_norm = l2_norm
        self.ln = nn.LayerNorm(in_dim) if layer_norm else None  
        self.small_init_proj_out = small_init_proj_out
        self.proj_out_init_std = proj_out_init_std

        if self.small_init_proj_out and isinstance(self.proj_out, nn.Linear):
            nn.init.normal_(self.proj_out.weight, mean=0.0, std=self.proj_out_init_std)
        ...
        if self.use_abs_pos_emb:
            # Absolute PE must have a fixed maximum length.
            # Prefer max_length if you already use it to truncate;
            # otherwise require pos_emb_max_length.
            pe_max_len = self.max_length if self.max_length and self.max_length > 0 else pos_emb_max_length
            if not pe_max_len or pe_max_len <= 0:
                raise ValueError(
                    "use_abs_pos_emb=True requires either max_length>0 or pos_emb_max_length>0 "
                    "so we can size the AbsolutePositionalEmbedding."
                )
            self.abs_pos_emb = AbsolutePositionalEmbedding(output_dim, pe_max_len)

    def forward(self, seqs: tp.List[torch.Tensor], device: tp.Union[torch.device, str]) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        # Move projection to device
        self.proj_out.to(device)
        if self.ln is not None:
            self.ln.to(device)
        if self.abs_pos_emb is not None: 
            self.abs_pos_emb.to(device)

        # Sanitize + move to device
        cleaned = []
        lengths = []
        ...
        for x in seqs:
            ...
            assert x.dim() == 2, f"Sequence embedding must be [T, D], got {tuple(x.shape)}"
            assert x.shape[1] == self.in_dim, f"Expected D={self.in_dim}, got {x.shape[1]}"

            # Optional truncation
            if self.max_length and x.shape[0] > self.max_length:
                x = x[:self.max_length]
        ...
        # If absolute PE is enabled, ensure we never exceed its max length
        if self.abs_pos_emb is not None:
            pe_max_len = self.abs_pos_emb.max_seq_len if hasattr(self.abs_pos_emb, "max_seq_len") else None
            # If AbsolutePositionalEmbedding doesn't expose max_seq_len, just rely on truncation above.
            if pe_max_len is not None and T_max > pe_max_len:
                # truncate to pe_max_len (and recompute)
                T_max = pe_max_len
                cleaned = [x[:T_max] for x in cleaned]
                lengths = [min(t, T_max) for t in lengths]

        # Pad
        dtype = next(self.proj_out.parameters()).dtype if not isinstance(self.proj_out, nn.Identity) else cleaned[0].dtype
        tokens = torch.zeros((B, T_max, self.in_dim), device=device, dtype=dtype)
        mask = torch.zeros((B, T_max), device=device, dtype=torch.bool)

        for i, x in enumerate(cleaned):
            t = x.shape[0]
            tokens[i, :t, :] = x.to(dtype)
            mask[i, :t] = True
            
        if self.ln is not None:
            tokens = self.ln(tokens)

        # Optional normalize (sometimes stabilizes attention)
        if self.l2_norm:
        ...
        # (so PE is in the same space the transformer sees)
        if self.abs_pos_emb is not None:
            # print('adding positional encoding')
            tokens = tokens + self.abs_pos_emb(tokens)

        return tokens, mask

class SequenceEmbeddingGlobalConditioner(Conditioner):
    """
    Global conditioner for embedding sequences.

    Input (from metadata): List[Tensor] where each Tensor is [T_i, in_dim] (usually in_dim=128).
    Output: tokens [B, 1, output_dim] and mask [B, 1] (all True).

    It optionally:
      - truncates to max_length
      - applies LayerNorm / L2 norm
      - projects every timestep: [B, T, in_dim] -> [B, T, output_dim]
      - pools across time (mean/max/attn) -> [B, output_dim]
    """
    def __init__(
        self,
        output_dim: int,
        ...
        if self.pool not in ("mean", "max"):
            raise ValueError(f"Unknown pool={self.pool}. Use 'mean' or 'max'.")

    def forward(
        self,
        seqs: tp.List[torch.Tensor],
        ...
        for x in seqs:
            ...
            if x.dim() != 2:
                raise ValueError(f"Sequence embedding input must be [T,D], got {tuple(x.shape)}")
            if x.shape[1] != self.in_dim:
                raise ValueError(f"Expected D={self.in_dim}, got {x.shape[1]}")
        ...
        B = len(cleaned)
        if B == 0:
            # edge-case: empty batch metadata (shouldn't happen)
            tokens = torch.zeros((0, 1, self.output_dim), device=device)
            mask = torch.zeros((0, 1), device=device, dtype=torch.bool)
            return tokens, mask

        T_max = max(lengths) if lengths else 1

        # Build padded tensor [B, T_max, in_dim] + mask [B, T_max]
        # Use float32 for stability unless you know you want fp16 here.
        tokens = torch.zeros((B, T_max, self.in_dim), device=device, dtype=torch.float32)
        mask = torch.zeros((B, T_max), device=device, dtype=torch.bool)

        for i, x in enumerate(cleaned):
            t = x.shape[0]
            # cast into tokens dtype
            tokens[i, :t, :] = x.to(tokens.dtype)
            mask[i, :t] = True

        # Optional LayerNorm on the feature dim (per token)
        if self.ln is not None:
            tokens = self.ln(tokens)

        # Optional L2 norm on feature dim (per token)
        if self.l2_norm:
            tokens = F.normalize(tokens, p=2, dim=-1)

        # Project every timestep: [B, T, in_dim] -> [B, T, output_dim]
        # If proj_out is Identity (dims match), this is a no-op.
        tokens_proj = self.proj_out(tokens)

        # Pool across time using mask
        if self.pool == "mean":
            denom = mask.sum(dim=1, keepdim=True).clamp_min(1).to(tokens_proj.dtype)  # [B,1]
            pooled = (tokens_proj * mask.unsqueeze(-1).to(tokens_proj.dtype)).sum(dim=1) / denom  # [B, output_dim]
        elif self.pool == "max":
            # set padded positions to -inf then max
            neg_inf = torch.finfo(tokens_proj.dtype).min
            masked = tokens_proj.masked_fill(~mask.unsqueeze(-1), neg_inf)
            pooled, _ = masked.max(dim=1)  # [B, output_dim]
        else:
            raise RuntimeError("unreachable")

        # Return as a single token sequence [B,1,output_dim]
        pooled = pooled.unsqueeze(1)
        pooled_mask = torch.ones((B, 1), device=device, dtype=torch.bool)
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any], pretransform=None) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type in ("sequence_embedding"):
            conditioners[id] = SequenceEmbeddingConditioner(**conditioner_config)
        elif conditioner_type in ("sequence_embedding_global"):
            conditioners[id] = SequenceEmbeddingGlobalConditioner(**conditioner_config)
        else:
