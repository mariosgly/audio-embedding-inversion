...
def create_training_wrapper_from_config(model_config, model):
    ...
    if model_type == 'autoencoder':
        ...
        return DiffusionCondTrainingWrapper(
            ...
            inpainting_config = training_config.get("inpainting", None),
            loss_config=training_config.get("loss_configs", None),
            memory_debug=training_config.get("memory_debug", False),
            memory_debug_first_n_steps=training_config.get("memory_debug_first_n_steps", 1),
            lora_config=training_config.get("lora_config", None),
            lora_full_train_param_patterns=training_config.get("lora_full_train_param_patterns", None),
            trainable_param_patterns=training_config.get("trainable_param_patterns", None),
        )
